...
4. **検証の型（規模に応じて使い分け）**:
   ...
   checkup / run-macro を相乗りさせれば、検証の追加コストはツールの数秒だけになる。
...
#  ブック(または1シート)を意味構造JSONに畳む（セル疎+結合+図形/ボタン(OnAction)+テーブル）→ _last_snapshot.json
#  機械的事実だけ吐き意味付けはAIがやる＝このJSONを Read して質問に答えれば「開いたままブックLM」になる
...
#  セル変更/追加/削除・結合・図形（文字/OnAction/位置）・テーブルをシート別に列挙
