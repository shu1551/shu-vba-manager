...
2. **点検の型**: list → export-all → コードを読む → 所見を数行で報告（詳細は聞かれたら）。
   checkup（健康診断）は利用者が明示的に求めたときだけ実行する（裁量で診断を回さない）。
...
4. **修理の型（「動かない」と言われたら・2026-07-17 制定）——考える工程を会話に出す**:
   - **名指しされたマクロは get 一発で取り、同じ返答の中で仮説まで言い切る**。
     「取った→次に読む→次に考える」と手数を割らない。探索（周辺調査・構造検分）を
     黙って積み上げない。利用者がコードを貼ってくれたら道具なしで即答する。
   - **仮説は一言で会話に出し、直し方を利用者に選ばせる**（「犯人はたぶん○○。
     呼び出し側に一手足すか、○○側を根本から直すか、どちらにしますか」）。
     最小の一手を先に提示する。大きい直しを黙って選ばない。
   - **利用者がその場にいるなら、検証は本人の実機テストが最速**（「直したので
     押してみてください」）。本人の10秒に勝てない自前のシミュレーション検証を組まない。
     ただし破壊系・非可逆の検証は従来どおり慎重に（安全ガードの撤廃ではない）。
   - 初手で聞くと速い: 「いつから動かない？」「最近何を変えた？」（修正前ファイルの
     存在や変更日は本人が一番速い）。
5. **検証の型（規模に応じて使い分け）**:
   - **1〜2プロシージャの軽微な修正**: `run-macro` 1本（コンパイル確認を兼ねる）だけで良い。
     checkup は付けない（診断は利用者が明示的に求めたときだけ）。
...
6. **公開の型**: `py publish_check.py <公開用コピー> --scrub` → commit/push → GitHub 生ファイルを
   ダウンロードして publish_check で再チェック（実測まで）。
7. **報告は結果数行が既定**。往復を減らす（独立な調査・実行は1往復に並列で畳む）。
