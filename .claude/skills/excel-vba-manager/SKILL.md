...
段取りをその場で考え直さず、この型どおり動く。**目的は往復回数の最小化**
（AI の作業時間はツールの実行時間ではなく、往復ごとの思考時間で決まる）。

1. **道具は MCP が第一選択**。vba_mcp_server.py を登録済みの環境では
   `vba("コマンド1行")` で全コマンドが撃てる（常駐 COM で応答 0.01〜0.2 秒級・
   シェルの引用符/エンコーディング事故なし）。確認プロンプトを出すコマンドは必ず `-y`。
   CLI（`py vba_manager.py …`）に落とすのは MCP 未登録の環境か、
   vba_manager 本体を改修して新コードを検証するとき（MCP はセッション起動時のコードを保持）だけ。
2. **点検の型**: `checkup`＋`list` → `export-all` → コードを読む → 所見を数行で報告（詳細は聞かれたら）。
   1,000 行を超えるブックはモジュールを分担して並列に読み、壁時計を圧縮する。
   対象モジュールが分かっているときの `list` は `--module 名` で絞る
   （実測: 182マクロ規模のブックで `list --detail` 全件は約3秒、`--module` 絞りはほぼ即応）。
3. **修正の型**: 置換コードを書く → `get`→`replace-procedure`（自動バックアップ・自動保存つき）。
   モジュール先頭行（`Option Explicit` 等）だけは `replace-module` で行う。
4. **検証の型（規模に応じて使い分け）**:
   - **1〜2プロシージャの軽微な修正**: `run-macro` 1本（コンパイル確認を兼ねる）だけで良い。
     `checkup` は付けない（実測: 182マクロ・所見32件規模のブックで `checkup` 単体が約7秒かかり、
     軽微修正の検証コストとして不釣り合い）。
   - **複数モジュールにまたがる修正・所見の増減を追いたい変更**: `checkup` 再実行（所見の増減）
     ＋ `run-macro` を同じ往復に相乗り。
   **検証を独立の往復にしない**——最後の置換と同じ往復に相乗りさせれば追加コストは数秒。
   小修正に過剰な検証を足さない。
5. **公開の型**（ブックを配布・公開するとき）: `py publish_check.py <公開用コピー> --scrub` で
   個人情報チェック（作成者実名/隠しシート/定義名/外部リンク/コメント/残留値）と
   メタデータ空欄化を1コマンドで。push 後は公開先の生ファイルを取得して再チェック（実測まで）。
   `--scrub` は公開用コピーにだけ使い、手元の原本には使わない。
6. **報告は結果数行が既定**。独立な調査・実行は1往復に並列で畳む。
...
py vba_manager.py check <basファイル>
py vba_manager.py printer-list / printer-setup <プリンタ名>
