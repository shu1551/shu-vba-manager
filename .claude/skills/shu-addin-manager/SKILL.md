...
```
秀.xlsm:            C:\Users\shu\Desktop\秀.xlsm（現用・2026-07-17 実測確認。
                    旧記載の「アプリ\秀 20260113\秀.xlsm」は現存しない）
shu003.bas:         SCRIPTS\shu003.bas（標準モジュール本体・正規ソース）
...
- **標準モジュール**: shu003（作業マクロ本体／103サブ・2026-07-17実測）, shu005（アドイン管理）,
  shu004（クリーン化と祝日更新）。shu001・shu002 は現在空。行数・サブ数は成長するので
  正確な今の姿は `list --module shu003` で実測する（この地図の数字を鵜呑みにしない）
